...
local privacy_width = #privacy_options * 145
local max_width = 800
...
local ServerSettingsTab = Class(Widget, function(self, slotdata, servercreationscreen)
    ...
    local num_visible_rows = math.floor(servercreationscreen:GetContentHeight() / label_height)
    ...
    self.intentions_overlay:SetPosition(-170, 230)
...
function ServerSettingsTab:ShowNewHostPicker(show)
    if show then
        if self.newhost_overlay == nil then
            ...
            self.newhost_overlay:SetPosition(-170, 180)
